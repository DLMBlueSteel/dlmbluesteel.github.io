...
function renderQueue() {
  ...
  list.querySelectorAll('.queue-item').forEach(item => {
    ...
    item.addEventListener('dragover', e => {
      ...
      if (draggedIndex !== null && draggedIndex !== parseInt(item.dataset.index)) {
    ...
    item.addEventListener('drop', async e => {
      ...
      if (draggedIndex !== null && draggedIndex !== toIndex) {
        await handleQueueReorder(draggedIndex, toIndex);
      }
      draggedIndex = null;
    });
  });
...
async function handleShuffle() {
  if (!selectedGuildId || queueData.length < 2) {
    showToast('Not enough songs to shuffle', 'info');
    return;
  }
  
  // Fisher-Yates shuffle
  const shuffled = [...queueData];
  ...
  queueData = shuffled;
  renderQueue();
  showToast(`Shuffled ${queueData.length} songs 🔀`, 'success');
}

// ─── Drag & Drop State ─────────────────────────
let draggedIndex = null;

async function handleQueueReorder(fromIndex, toIndex) {
  if (!selectedGuildId || fromIndex === toIndex) return;
  if (toIndex < 0 || toIndex >= queueData.length) return;
  
  try {
    const [item] = queueData.splice(fromIndex, 1);
    queueData.splice(toIndex, 0, item);
    renderQueue();
...
function toggleFavorite(track) {
  const idx = favorites.findIndex(f => f.url === track.url);
  if (idx >= 0) {
    favorites.splice(idx, 1);
    showToast('Removed from favorites ♡', 'info');
  } else {
    favorites.unshift({ ...track, addedAt: new Date().toISOString() });
...
function isFavorite(trackUrl) {
  return favorites.some(f => f.url === trackUrl);
}

function updateFavoriteButton() {
  const btn = $('bbFavoriteBtn');
  if (!btn || !nowPlayingData) return;
  const isFav = isFavorite(nowPlayingData.url || '');
...
function renderFavorites() {
  ...
  container.innerHTML = favorites.map((track, i) => `
    <div class="recent-card">
      <div class="recent-time-badge">${new Date(track.addedAt || 0).toLocaleDateString()}</div>
      <div class="recent-art">
        ${track.thumbnail ? `<img src="${esc(track.thumbnail)}" alt="" loading="lazy" />` : '<div style="background:rgba(255,255,255,0.1);width:100%;height:100%;display:flex;align-items:center;justify-content:center;"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M9 18V5l12-2v13"/><circle cx="6" cy="18" r="3"/><circle cx="18" cy="16" r="3"/></svg></div>'}
        <div class="recent-play-hover">
          <button class="fav-remove-btn" data-index="${i}" title="Remove from favorites" style="background:none;border:none;color:#ff6b6b;cursor:pointer;padding:6px;border-radius:50%;background:rgba(0,0,0,0.5);">
            <svg viewBox="0 0 24 24" fill="currentColor" stroke="none" width="24" height="24"><path d="M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12 19 6.41z"/></svg>
          </button>
        </div>
      </div>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Favorite button
  const favBtn = $('bbFavoriteBtn');
  if (favBtn) {
    favBtn.addEventListener('click', () => {
      if (nowPlayingData) toggleFavorite(nowPlayingData);
    });
  }
