...
function setView(name) {
  ...
  const titles = { player: 'Player', queue: 'Queue', search: 'Add Song', 'user-settings': 'User Settings', settings: 'Settings' };
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Effects: update the bot immediately when the slider is released.
  const effectsBtn = $('effectsBtn'), effectsPanel = $('effectsPanel');
  if (effectsBtn && effectsPanel) {
    effectsBtn.addEventListener('click', e => { e.stopPropagation(); effectsPanel.hidden = !effectsPanel.hidden; });
    document.addEventListener('click', e => { if (!effectsPanel.contains(e.target) && e.target !== effectsBtn) effectsPanel.hidden = true; });
  }
  function bindEffect(sliderId, valueId, endpoint) {
    const slider = $(sliderId), value = $(valueId);
    if (!slider) return;
    slider.addEventListener('input', () => { value.textContent = Number(slider.value).toFixed(2) + 'x'; });
    slider.addEventListener('change', async () => {
      if (!selectedGuildId) { showToast('Select a server first', 'error'); return; }
      try {
        await apiFetch(`/api/${endpoint}?guildId=${selectedGuildId}&val=${slider.value}`, { method: 'POST' });
        showToast(`${endpoint === 'speed' ? 'Speed' : 'Pitch'} set to ${Number(slider.value).toFixed(2)}x`, 'success');
      } catch (err) { showToast(`Could not update ${endpoint}: ${err.message}`, 'error'); }
    });
  }
  bindEffect('speedSlider', 'speedValue', 'speed');
  bindEffect('pitchSlider', 'pitchValue', 'pitch');

  // British English is the default. This preference is strictly local to this browser.
  ...
  function applyLocale(locale) {
    ...
    document.querySelectorAll('[data-british][data-american]').forEach(el => {
      el.textContent = british ? el.dataset.british : el.dataset.american;
    });
  ...
  // A selected image is stored only in this browser; it is never sent to the bot/API.
  ...
  function applyLocalBackground(image) {
    if (!scene) return;
    scene.classList.toggle('custom-background', !!image);
  ...
  if (clearBackgroundBtn) clearBackgroundBtn.addEventListener('click', () => {
    ...
    showToast('Local background cleared', 'info');
  });

  // Accent colours are local preferences and update the whole CSS gradient.
  ...
  function applyAccent(start, end) {
    ...
    localStorage.setItem('dlm_accent_start', start);
    localStorage.setItem('dlm_accent_end', end);
  }
